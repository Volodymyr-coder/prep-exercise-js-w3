...
const getPeopleOfClass = (className) => {
  const getNameClass = classes.find((item) => item.name === className);

  const studentAndMentors = [];
  ...
  for (const student of students) {
    if (student.class === className) {
      ...
      studentAndMentors.push(newStudent);
    }
  }

  for (const mentor of mentors) {
    if (mentor.nowTeaching === getNameClass.currentModule) {
      const newMentor = {
        name: mentor.name,
        role: 'mentor',
      };
      studentAndMentors.push(newMentor);
    }
  }

  return studentAndMentors;
};
...
const getActiveClasses = () => {
  const nameActiveClasses = classes
    .filter((item) => {
      return item.active;
    })
    .map((item) => {
  ...
  const allActiveClasses = { ...nameActiveClasses };
  nameActiveClasses.forEach((item) => {
    allActiveClasses[item] = getPeopleOfClass(item);
  });
